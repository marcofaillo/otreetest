...
class C(BaseConstants):
    NAME_IN_URL = 'Risk_Mitigation_3rd_inv'
    EXCHANGE_RATE=0.02
...
class Player(BasePlayer):
    ...
    q2_2 = models.IntegerField(choices=[[1, '£3 (50 tokens x £0.06)'], [2, '£6 (50 tokens x £0.12)'], [3, '£0 (50 tokens x £0.0)']], initial=0)
    q2_3 = models.IntegerField(choices=[[1, '£0(50 tokens x £0) or £6 (50 tokens x £0.12) depending on the return for uninsured tokens'], [2, '£0 (50 tokens x £0)'], [3, '6£ (50 tokens x 0.12£)']],initial=0)
    ...
    q1_2 = models.IntegerField(choices=[[1, '£2 (50 tokens x £0.04)'],[2, '£8 (50 tokens x £0.16)'], [3, '£3 (50 tokens x £0.06)'], ], initial=0)
    q1_3 = models.IntegerField(choices=[[1, '£2 (50 tokens x £0.04)'], [2, '£8 (50 tokens x £0.16) or £2 (50 tokens x £0.04) depending on return for uninsured tokens'], [3, '3£ (50 tokens x 0.06£)']],initial=0)
...
def set_payoff(player: Player):
    ...
    if player.paid_decision== 2: #decision 2 is selected
        if player.lottery_2 ==1: # loss = 0.16
           player.payoff=(50-player.insure_2)*0.0+(player.insure_2)*0.06
        else: # loss = 0.04
           player.payoff=(50-player.insure_2)*0.12+(player.insure_2)*0.06
    if player.paid_decision== 1: #decision 1 is selected
        if player.lottery_1==1: # noloss
           player.payoff=(50-player.insure_1)*0.16+(player.insure_1)*0.06
        else: # loss =0.12
           player.payoff=(50-player.insure_1)*0.04+(player.insure_1)*0.06
...
class Questions_2(Page):
    ...
    def error_message(player: Player, values):
        solutions = dict(q2_1=3, q2_2=1, q2_3=1)
        errors = {f: 'Wrong' for f in solutions if values[f] != solutions[f]}
        if errors:
            player.errors_2 += 1
